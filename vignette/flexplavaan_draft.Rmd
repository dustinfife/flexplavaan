...
significance = ifelse(fit[["pvalue"]]>0.05, " not statistically significant", " is statistically significant")
```
...
data(exercise_data, package = "flexplot")
...
```{r traceMatrix, fig.cap="Scatterplot matrix showing the model-implied fit (red) and loess lines (blue) between three simulated indicator variables. The diagonals show the histograms of the ICRs."}
require(flexplavaan)
...
\end{tikzpicture}
\caption{Nonlinear model for the Jedi dataset. }
...
Figure \@ref(fig:tracenl) shows the trace/DDPs for the two models. As before, the red line is the original model, while the blue line shows the nonlinear model for the variables saber, fitness, midichlorian, and force_history. There are some minor discrepancies (e.g., the saber/midichlorian relationship), with the nonlinear model suggesting stronger relationships between the variables. However, as before, these plots are not terribly sensitive to misspecification. 
...
On the other hand, the measurement plots (Figure \@ref(fig:measnl)) do show differences between the two models, at least for the force_score latent variable. The nonlinear model shows consistently weaker relationships between the force variable and the indicators shown (force history, exam one, exam two, and exam three). 


```{r measnl, fig.cap="Measurement plots for the force_score latent variable, with both the nonlinear, red, model and original, blue, model."}
