...
fit.bayes.nonlinear <- readRDS("data/initial_bayes_fit_nonlinear.rds")
  ...
  ## factor scores
d$f_linear = as.numeric(predict(fit.bayes.linear))
d$f_nonlinear = as.numeric(predict(fit.bayes.nonlinear))
...
diagnostic_plots("x1", "x2", "f_linear", data=d, object=fit.bayes.linear, plot="residuals")
