...
#plot(magic_skills, survived)
  ...
  ### knowledge questions
potions = 70 + .7*(15)*magic_knowledge + rnorm(n, 0, sqrt(1-.5^2) * 15); potions =potions%>% ceiling_floor
history = 70 + .8*(15)*magic_knowledge + rnorm(n, 0, sqrt(1-.5^2) * 15); history =history%>% ceiling_floor
herbology = 70 + .5*(15)*magic_knowledge + rnorm(n, 0, sqrt(1-.5^2) * 15); herbology =herbology%>% ceiling_floor

  ### abilities
darkarts = 10+logistic(magic_skills, 80, 2.5, -1.250) + rnorm(n, 0, 7); darkarts = darkarts%>% ceiling_floor
