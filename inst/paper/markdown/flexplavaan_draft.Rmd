...
To motivate our discussion/explanation of visualizing LVMs, we begin with a simulated dataset. Suppose the Jedi council is attempting to identify padawans who will make good jedi knights. To do so, they develop seven indicators (light saber score, fitness score, midichlorian levels, and a jedi history exam, as well as three written exams completed at the end of jedi training). The jedi council posits that these indicators represent two latent factors (Force Propensity and Jedi Expertise), according to the relationships specified in Figure \@ref(fig:force). Notice that one variable (history) has cross loadings on both factors. 

In our examples that follow we will demonstrate how flexplavaan is able to detect both well-specified and poorly-specified models. 
<br>
<center>
...
\node[draw, circle] (force) {Force Propensity};
\node[draw, circle, node distance = 2cm] (jedi) [right of=force] {Jedi Expertise};
...
require(flexplavaan)
visualize(force_fit, plot = "model", sample=300, subset=1:3)
```
